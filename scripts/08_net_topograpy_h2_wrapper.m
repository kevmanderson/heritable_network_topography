...
base_dir = '/gpfs/milgram/scratch/holmes/topo_herit/data/topology_heritability'
base_dir = '/gpfs/milgram/project/holmes/kma52/topo_herit/data/topology_heritability/'
% HCP kinship matrix
kin   = csvread(fullfile(base_dir, 'K.csv'), 1);
...
for (i = 1:3)
    ...
    K=kin; P=pheno; X=covar;
    %[h2, p_perm] = h2_mat(pheno, kin, covar, fam, 1000);
    [h2, p_perm, jack_se] = h2_mat(pheno, kin, covar, 10, fam);
    out_mat(1,:) = {h2, p_perm, jack_se, 'overall', hemi};
...
% HCP covariates
X = csvread('covar.csv', 1);

% Family IDs
F = csvread('F.csv', 1);

% Dice similarity
P = csvread('lh_overall_dice_P.csv', 1);

% calc h2
[h2, p_perm] = h2_mat(P, K, X, F, 1000);
